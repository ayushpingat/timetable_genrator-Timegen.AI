...
function saveConfigToStorage() {
  const noOfDiv = document.getElementById("configNoOfDiv").value;
  const workingDays = document.getElementById("configWorkingDays").value;
  const periodsPerDay = document.getElementById("configPeriodsPerDay").value;
  const labCount = document.getElementById("configLabCount").value;
  const lectureDuration = document.getElementById("configLectureDuration").value;
  const practicalDuration = document.getElementById("configPracticalDuration").value;

  if (!noOfDiv || !workingDays || !periodsPerDay || !labCount || !lectureDuration || !practicalDuration) {
    alert("Please fill in all configuration fields");
    return;
  }

  const config = {
    noOfDiv: Number(noOfDiv),
    workingDays: Number(workingDays),
    periodsPerDay: Number(periodsPerDay),
    labCount: Number(labCount),
    lectureDuration: Number(lectureDuration),
    practicalDuration: Number(practicalDuration),
    startTime: "09:00",
    endTime: "17:00",
    lunchBreak: 60,
    shortBreak: 5,
...
function loadSubjects() {
  ...
  subjects.forEach((s, i) => {
    ...
    const hoursInfo = `T:${s.theory || 0}h, P:${s.practical || 0}h, Tu:${s.tutorial || 0}h`;
    const hoursStatus = (s.theory || 0) + (s.practical || 0) + (s.tutorial || 0) === 0 
      ? '<span style="color: red; font-weight: bold;">❌ NO HOURS</span>' 
      : `<span style="color: green;">✓ ${hoursInfo}</span>`;

    div.innerHTML = `
      <div>
        <strong>${s.subject}</strong><br>
        Faculty: ${s.faculty.length > 0 ? s.faculty.join(", ") : "Not Assigned"}<br>
        Hours: ${hoursStatus}
      </div>
      <div>
        <button onclick="editSubjectHours(${i})">⏱️ Hours</button>
        <button onclick="showFaculty(${i})">Assign</button>
        <button class="danger-small" onclick="deleteSubject(${i})">🗑️</button>
      </div>
    `;
...
function editSubjectHours(index) {
  const subjects = JSON.parse(localStorage.getItem("subjects")) || [];
  if (index < 0 || index >= subjects.length) return;

  const subject = subjects[index];
  
  const theory = prompt(`${subject.subject}\n\nEnter Theory Hours per week (current: ${subject.theory || 0}):`, subject.theory || 0);
  if (theory === null) return;
  
  const practical = prompt(`${subject.subject}\n\nEnter Practical Hours per week (current: ${subject.practical || 0}):`, subject.practical || 0);
  if (practical === null) return;
  
  const tutorial = prompt(`${subject.subject}\n\nEnter Tutorial Hours per week (current: ${subject.tutorial || 0}):`, subject.tutorial || 0);
  if (tutorial === null) return;

  try {
    subject.theory = Math.max(0, parseInt(theory) || 0);
    subject.practical = Math.max(0, parseInt(practical) || 0);
    subject.tutorial = Math.max(0, parseInt(tutorial) || 0);
...
function sendDataToBackend() {
  ...
  const invalidSubjects = validSubjectsUI.filter(s => s.theory === 0 && s.practical === 0 && s.tutorial === 0);
  ...
  const payload = {
    ...
    subjects: validSubjectsUI.map(ui => {
      ...
      return {
        ...
        theory_hours: ui.theory,
        practical_hours: ui.practical,
        tutorial_hours: ui.tutorial
  ...
  .then(data => {
    ...
    // Store timetable data in sessionStorage for results page
    if (data.timetables && data.days) {
      const timetableData = {
    ...
    alert("Timetable generated but data format unexpected.");
  })
  .catch(err => {
    console.error(err);
